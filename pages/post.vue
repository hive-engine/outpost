...
export default {
  ...
  mounted () {
    ...
    this.$eventBus.$on(['post-mute-successful', 'comment-mute-successful'], this.onMute)
  },
  ...
  beforeDestroy () {
    ...
    this.$eventBus.$off(['post-mute-successful', 'comment-mute-successful'], this.onMute)
  },
  ...
  methods: {
    ...
    async onMute ({ authorperm }) {
      try {
        await this.sleep(30 * 1000)

        const authorpermWithoutAt = authorperm.substring(1)

        const [author, permlink] = authorpermWithoutAt.split('/')

        const content = await this.fetchPost({ author, permlink })

        if (content) {
          delete content.json_metadata

          const newData = {
            ...this.discussions[authorpermWithoutAt],
            ...content
          }

          this.discussions[authorpermWithoutAt] = newData
        }
      } catch {
        //
      }
