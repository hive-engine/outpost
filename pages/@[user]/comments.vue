  ...
  <div class="user-comments">
    <b-container fluid="lg">
      ...
      <b-card v-else class="mt-5">
        Looks like @{{ $route.params.user }} hasn't commented yet!
      </b-card>
    </b-container>
  </div>
</template>
...
<script>
// Ported from legacy/pages/_user/comments.vue (route name 'user-comments').
...
const endpoint = 'get_discussions_by_comments'
...
export default {
  ...
  async setup () {
    definePageMeta({ name: 'user-comments' })

    useHead({ title: 'Comments by' })

    const route = useRoute()
    const scot = useScotStore()

    const { data: initialPosts } = await useAsyncData(
      `user-comments-${route.params.user}`,
