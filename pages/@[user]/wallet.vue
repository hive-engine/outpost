<template>
  <div class="user-wallet">
        ...
        <b-card class="mt-3">
          <b-row no-gutters>
            ...
            <b-col cols="4" class="text-end">
              <b-dropdown variant="link" end :disabled="disableDropdown" :no-caret="disableDropdown">
          ...
          <b-row no-gutters>
            ...
            <b-col cols="4" class="text-end">
              ...
              <div v-if="tokenNetDelegation !== 0" class="h6 px-3">
                {{ tokenNetDelegation > 0 ? `+${numberWithCommas(tokenNetDelegation)}` : numberWithCommas(tokenNetDelegation) }} {{ config.TOKEN }} <a v-if="tokenDelegations.length > 0" class="small" href="#" @click.prevent="ui.showModal('delegationsModal')">
                  <fa-icon icon="eye" />
                </a>
              </div>
            </b-col>
    ...
    <b-modal id="actionModal" v-model="ui.modals.actionModal" centered :title="modalTitle">
      ...
      <b-row>
        ...
        <b-col cols="3" md="2" class="text-end fw-bold">
          {{ percentage }}%
        </b-col>
      </b-row>
...
<script>
// Ported from legacy/pages/_user/wallet.vue (route name 'user-wallet').
// fetch() -> async load() in created() (data-mutating methods kept); head() -> useHead;
// $config -> config, $auth -> auth (setup); $bvModal.show/hide -> useUiStore().showModal/hideModal;
// $root.$on('bv::modal::hidden') -> watch(ui.modals.actionModal); $axios -> $api (plugin);
// Vuex getters/actions -> Pinia mapState/mapActions; <timeago> -> local Timeago drop-in.
// BS5: text-right -> text-end, font-weight-bold -> fw-bold, dropdown right -> end.
...
export default {
  ...
  setup () {
    definePageMeta({ name: 'user-wallet' })

    useHead({ title: 'Wallet of' })
  ...
  watch: {
    ...
    // Legacy $root.$on('bv::modal::hidden') for 'actionModal' -> watch the modal's v-model.
    'ui.modals.actionModal' (open) {
      if (!open) {
        this.available = 0
        this.amount = 0
        this.percentage = 0
