        ...
        <template v-if="following.length > 0">
          <div v-for="(follower,i) of following" :key="i" class="mt-3 pb-3 border-bottom">
            <b-row>
              ...
              <b-col cols="6" sm="3" class="text-sm-end">
                <div class="small text-muted">
                  Vote Value
                </div>
...
<script>
// Ported from legacy/pages/_user/following.vue (route name 'user-following').
// fetch() -> async setup() + useAsyncData; head() -> useHead;
// vue-infinite-loading -> ~/components/app/InfiniteLoading.vue;
// tribe_info/tribe_config -> mapState(useTribeStore); $config -> config (setup).
...
export default {
  ...
  async setup () {
    definePageMeta({ name: 'user-following' })

    useHead({ title: 'Followed by' })
