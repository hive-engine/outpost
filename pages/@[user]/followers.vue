            ...
            <b-row>
              <b-col sm="6">
                <div class="d-flex align-items-center">
                  <b-avatar :src="`${config.IMAGES_CDN}u/${follower.username}/avatar`" variant="dark" size="48px" class="me-3" />
...
<script>
// Ported from legacy/pages/_user/followers.vue (route name 'user-followers').
// fetch() -> async setup() + useAsyncData; head() -> useHead;
// vue-infinite-loading -> ~/components/app/InfiniteLoading.vue;
// Vuex root getters tribe_info/tribe_config -> mapState(useTribeStore);
// $config -> config (setup); $chain/$sidechain stay this.$x (plugin provides).
...
export default {
  ...
  async setup () {
    definePageMeta({ name: 'user-followers' })

    useHead({ title: 'Followers of' })
  ...
  computed: {
    ...mapState(useTribeStore, ['tribe_info', 'tribe_config'])
  },
