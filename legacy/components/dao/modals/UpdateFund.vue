<template>
  <b-modal id="updateFundModal" centered size="lg" title="Update DAO">
    ...
    <template #modal-footer>
      <b-button variant="primary" @click.prevent="updateFund">
        Update
      </b-button>
    </template>
  </b-modal>
...
export default {
  ...
  mounted () {
    ...
    this.$eventBus.$on('dao-update-successful', this.onDaoUpdate)
  },

  beforeDestroy () {
    this.$root.$off('bv::modal::show', this.onModalShow)
    this.$eventBus.$off('dao-update-successful')
  },
  ...
  methods: {
    ...
    onDaoUpdate () {
      this.$bvModal.hide('updateFundModal')
    }
  },
