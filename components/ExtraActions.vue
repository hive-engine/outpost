    ...
    <div class="d-flex align-items-center">
      <div class="mr-2">
        <b-dropdown variant="link" no-caret dropup>
          ...
          <b-dropdown-item-button v-if="$auth.loggedIn && $auth.user.username === muting_account" @click="requestMutePost({type, authorperm: post.authorperm, mute: !muted})">
            {{ muted? 'Unmute' : 'Mute' }}
          </b-dropdown-item-button>
...
export default {
  ...
  computed: {
    ...mapGetters(['tribe_config', 'tribe_info', 'muting_account']),
  ...
  methods: {
    ...mapActions('post', ['requestBroadcastReblog', 'requestEditPost', 'requestBroadcastDelete', 'requestMutePost', 'requestPromotePost']),
        ...
        const messageVNode = h('div', { class: [''] }, [
          ...
          h('b-form-group', { class: ['mt-3'], props: { label: 'Amount' } }, [
            h('b-input-group', { props: { append: this.$config.TOKEN } }, [
              h('b-form-input',
                {
                  props: { number: true, type: 'number', value: self.amount, required: true },
