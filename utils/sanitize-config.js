...
import { isDefaultImageSize, defaultSrcSet } from '@/utils/proxify-url'
import { getPhishingWarningMessage, getExternalLinkWarningMessage } from '@/utils/HtmlReady'
import { validateIframeUrl as validateEmbbeddedPlayerIframeUrl } from '@/utils/embeds'
...
export default ({
  ...
  transformTags: {
    ...
    a: (tagName, attribs) => {
      ...
      if (!href.match(new RegExp(`^(/(?!/)|${APP_DOMAIN})`))) {
